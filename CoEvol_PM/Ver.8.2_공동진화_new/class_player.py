...
########################################### SENSOR FUCTIONS #############################################
    ...
    def p_learning_process(self):
        ...
        before_output = outputs

        changed = False
        while True:
            n_pos =self.grid_pos+direcs[outputs]
            if not self.find_wall(n_pos):
                self.direction = direcs[outputs]
                break
            changed = True
            if outputs == 3:
                outputs = 0
        ...
        #
        # if changed:
        #     if self.Game.fps_after_start % 5 == 0:
        #         print("----보정 전----",di[before_output],"----보정 후----",di[outputs])
        ...
        #
        # di = [ "→", "←", "↑",  "↓" ]
        # if self.Game.fps_after_start % 10 == 0:
        #     if changed:
        #         print("----보정 전----",before_output)
